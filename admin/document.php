...
if (isset($_GET['third_party_c'])) {
    ...
    try {
        ...
        echo $third_party_c = base64_decode($_GET['third_party_c']);

        if (!$third_party_c) {
            throw new Exception("Invalid ID after decoding.");
        }

        // Debug: Check if record exists first
        $check = $db->prepare("SELECT id, status FROM patients_documents WHERE id = ?");
        $check->execute(array($third_party_c));
        $row = $check->fetch(PDO::FETCH_ASSOC);

        if (!$row) {
            throw new Exception("Record not found for ID: " . $third_party_c);
        }
        ...
        if ($update->rowCount() == 0) {
            throw new Exception("No rows affected. Status may already be 0.");
        }
...
$stmt = $db->query("SELECT e.*,i.insurance_name,i.interest,i.insurance_type,i.add_minus FROM enrollee as e 
	INNER JOIN insurance_tbl as i ON e.hmo_no=i.insurance_no WHERE hospital_no='$token' and status='active'");
if ($stmt->rowCount() > 0) {
                    ...
                    <div class="col-lg-12">
                        <div class="ibox ">
                            <div class="ibox-title">
                                <h5>
                                    <a style="float:right">
                                        <a href="index.php?ptm=all/<?= $token; ?>" class="btn btn-sm btn-primary">Return to Patient Page</a>
                                    </a>
                                    Patient Dashboard
                                </h5>
                            </div>
                            ...
                            <div class="ibox-content">
                                <div class="row">
                                                        ...
                                                        ?>
                                                            <tr>
                                                                ...
                                                                <th>
                                                                    ...
                                                                    ?>
                                                                        <a href="document.php?emr=<?= $hospital_no; ?>&third_party_c=<?= base64_encode($patient_doc->id); ?>"
                                    ...
                                    <div class="col-sm-4">
                                        <?php
                                        /// include_once('_patient_dashboard_profile.php');
                                        ?>
                                    </div>
                                </div>
