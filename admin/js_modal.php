...
if ($price == 'c') {
    ...
    $title = "Pharmacy";
    $table_type = 'stock_table';
    $dept_srch = "(department_type='medical services' )";
} elseif ($price == 'cn') {
    $title = "Nursing Consumable";
    $table_type = 'stock_table';
    $dept_srch = "(department_type='medical services' )";
} elseif ($price == 'iv') {
...
<div class="modal inmodal fade" id="edit_bed_tariff_modal" tabindex="-1" role="dialog" aria-hidden="true" data-keyboard="false" data-backdrop="static">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body" id="edit_bed_tariff_body">
            </div>
...
<div class="modal inmodal fade" id="download_modal" tabindex="-1" role="dialog" aria-hidden="true" data-keyboard="false" data-backdrop="static">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                <h4 class="modal-title" id="">Download Price List</h4>
            </div>

            <div class="modal-body">
                <form action="excel_price_download.php" method="POST" id="download_stock_form">

                    <div class="form_sep">
                        <button class="btn btn-primary" type="submit" name="download_the_stock">Download Price Item</button>
                    </div>
...
<div class="modal inmodal fade" id="upload_modal" tabindex="-1" role="dialog" aria-hidden="true" data-keyboard="false" data-backdrop="static">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                <h4 class="modal-title" id="">Upload Price List</h4>
            </div>

            <div class="modal-body">
                <form action="" id="price_upload_form" method="POST" enctype="multipart/form-data">
                    <div class="form_sep">
                        <label for="">Select excel file</label>
                        <input type="file" name="the_upload" class="form-control">
                    </div>
                    <div class="form_sep" id="upload_the_price_spinner" style="display: none;">
                        <div class="spinner-border" role="status">
                            <span>Loading...</span>
                        </div>
                    </div>
                    <div id="err_price_upload"></div>
                    <div class="form_sep" id="">
                        <br><input class="btn btn-primary btn-xs" value="Upload" type="submit" name="upload_the_price" id="upload_the_price_btn">
                    </div>

                    <input type="hidden" name="price__" value="<?php echo $price ?>" />
                </form>
            </div>
        </div>
    </div>
...
<script>
    $(document).on('click', '#upload_price_btn', function() {
        //	$('.modal-title').text('Patient Medical Report'); 
        $('#upload_modal').modal('show');
    });

    $(document).on('click', '#download_price_btn', function() {
        //	$('.modal-title').text('Patient Medical Report'); 
        $('#download_modal').modal('show');
    });

    $(document).ready(function(e) {
        $("#price_upload_form").on('submit', (function(e) {
            e.preventDefault();
            $('#upload_the_price_btn').prop("disabled", true);
            $('#upload_the_price_spinner').show();
            $.ajax({
                url: "excel_price_download.php",
                type: "POST",
                ...
                beforeSend: function() {
                    //$("#preview").fadeOut();
                    $("#err_price_upload").fadeOut();
                },
                success: function(data) {
                    console.log(data);
                    if (data == 'invalid') {
                        // invalid file format.
                        $("#err_price_upload").html("Invalid File !").fadeIn();
                    } else {
                        // console.log('done');
                        ...
                        $('#upload_the_price_btn').prop("disabled", false);
                        $('#upload_the_price_spinner').hide();
                        $('#upload_modal').modal('hide');
                    }
                },
                error: function(e) {
                    $("#err_price_upload").html(e).fadeIn();
                }
            });
        }));
    });
...
<div class="modal inmodal fade" id="add_specialist_modal" tabindex="-1" role="dialog" aria-hidden="true" data-keyboard="false" data-backdrop="static">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                <h4 class="modal-title" id="">Add Specialist</h4>
            </div>

            <div class="modal-body" id="add_specialist_body" style="height: 300px">
...
<div class="modal inmodal fade" id="admin_settings_modal" tabindex="-1" role="dialog" aria-hidden="true" data-keyboard="false" data-backdrop="static">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                <h4 class="modal-title" id="">Admin Settings</h4>
            </div>

            <div class="modal-body" id="admin_settings_body">
