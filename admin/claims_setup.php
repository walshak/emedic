...
if (isset($_POST['post_claims']) && !empty($_REQUEST['inv'])) {
	...
	try {
		...
		foreach ($pro_inv as $inv_id) {
			...
			// Get service group
			$stmtChk = $db->prepare("SELECT serv_group,paystatus,invoice_status FROM patient_ap_services WHERE hospital_no = :hn AND sn = :sn LIMIT 1");
			$stmtChk->execute(array(':hn' => $hos_no, ':sn' => $sn));
			$serv = $stmtChk->fetch(PDO::FETCH_ASSOC);
			$serv_group = $serv['serv_group'];;
			$paystatus_pharm = $serv['paystatus'];;
			//$invoice_status = isset($serv['invoice_status']) ? $serv['invoice_status'] : 1;
	...
	if ($sn != ''):
		...
		if ($type == 'vld') {
			...
			if (isset($_GET['special_package'])) {
				...
				//// SPECIAL PACKAGE CODE LOADING HERE .........................................

				$paystatus = 1;
				$invoice_status = 1;
				$pay_mode = 'spkage';
				$claim_valid_by = $_SESSION['fullname'] . '/<br>' . $Current_date = date('d-m-y h:i:s a');
				$transc_date = date("Y-m-d H:i:s");
			...
			SET paystatus = :paystatus, 
				...
				claim_valid_by = :claim_valid_by, 
				pay_mode = :pay_mode 
			WHERE hospital_no = :hospital_no AND sn = :sn AND paystatus = 0');
				$stmt->execute([
					':paystatus' => $paystatus,
					':invoice_status' => $invoice_status,
					':transact_date' => $transc_date,
				...
				// Override for pharmacy group if unpaid
				if ($rwx['paystatus'] == '1') {
					$paystatus = 1;
					$invoice_status = 1;
				} elseif ($rwx['serv_group'] == 'Pharmacy') {
					$paystatus = 0;
					$invoice_status = 0;
				}
				...
				if (empty($invoice_by) && $rwx['invoice_status'] == 0) {
					$invoice_by = $_SESSION['fullname'];
					$invoice_date = $now;
					$invoice_no = date('m') . sprintf('%06d', mt_rand(0, 999999));
				}

				if ($rwx['invoice_status'] == 0) {
					$invoice_posted_by = $_SESSION['fullname'];
				}
		...
		if ($type === 'dvl') {
			...
			// If same user who invoiced, also reset billing info
			if ($rwx['invoice_by'] === $_SESSION['fullname']) {
				$baseQuery .= ", paystatus = 0, invoice_status = 0";
			}

			$baseQuery .= " 
			WHERE hospital_no = :hos_no 
			AND sn = :sn 
			AND (drug_status = '0' OR drug_status = '')";
		...
		if ($type === 'cvtP') {
			// Fetch safely
			$stmt = $db->prepare("
			SELECT hosp_price, cat_type, serv_group,qty,invoice_status
			...
			if ($rwx) {
				$Amount_payable = $rwx['hosp_price'] * $rwx['qty']; // multiply by quantity
				$invoice_status = $rwx['invoice_status'];
				//$invoice_status = ($rwx['serv_group'] === 'Pharmacy') ? '0' : '1';
		...
		if ($type === 'cvtPP') {
			// Fetch safely
			$stmt = $db->prepare("
			SELECT hosp_price, cat_type, serv_group 
			FROM patient_ap_services 
			WHERE sn = :sn");
			$stmt->execute([':sn' => $sn]);
			$rwx = $stmt->fetch(PDO::FETCH_ASSOC);
		...
		if ($type == 'cvtC') {
			...
			$stmt = $db->prepare("SELECT i.interest, i.insurance_type, i.insurance_name, i.insurance_no , i.payment_mode 
							  FROM enrollee AS e 
			...
			if ($stmt->rowCount() > 0) {
				...
				if ($amt_paying > 0) {
					/// calculation has beed done already 
					$claim_amt = $amt_paying;
				} else {
					...
					switch (true) {
						...
						case in_array($serv_group_upper, ['PHARMACY', 'CONSUMABLE', 'CONSUMABLES']):
							$NHIS_DRUG_CONSUMBL_STATE = 1;
							$_tariff_table = "hmo_stocks_tariff";
							break;

						case in_array($serv_group, ['Laboratory', 'Radiology']):
							$_tariff_table = "hmo_investigation_tariff";
							$stmt_g = $db->prepare("SELECT test_id FROM lab_manage WHERE labrequest_no = :target_sn");
							$stmt_g->execute([':target_sn' => $target_sn]);

							if ($stmt_g->rowCount() > 0) {
								$rwxx55 = $stmt_g->fetch(PDO::FETCH_ASSOC);
								$target_sn = $rwxx55['test_id'];
							}
							break;

						case ($cat_type === 'Bed Space/Accommodation'):
					...
					$target_sn = $item_sn;
					$serv_group_upper = strtoupper(trim($serv_group));
					$price_calc_file = "../inc/price_calc.php";
					include_once $price_calc_file;
				}


				$pay_mode   = 'claim';
				$transc_date = date("Y-m-d");
				$claim_valid_by = $_SESSION['fullname'] . ' / ' . date('d-m-y h:i:s a');

				if ($claim_amt == 0 && $hosp_price == 0 && $amt_paying > 0) {
					$claim_amt = $amt_paying;
				}
				...
				if ($claim_amt > 0) {
					$updateSQL = "UPDATE patient_ap_services 
						  SET paystatus = :paystatus,
							  ...
							  pay_mode = :pay_mode,
							  claim_valid_by = :claim_valid_by
						  WHERE hospital_no = :hos_no AND sn = :sn AND paystatus = 0";
...
if (isset($_POST['update_auth'])) {
	...
	$appt_no   = trim($_POST['appt_no']);

	// Update authorization code
	$stmt = $db->prepare('UPDATE apptm SET auth_code = :auth_code WHERE appt_no = :appt_no');
	$stmt->bindParam(':auth_code', $auth_code);
	$stmt->bindParam(':appt_no', $appt_no);
	$stmt->execute();

	// Redirect with proper GET params
	$from = isset($from) ? $from : '';
	$to   = isset($to) ? $to : '';
	$hos_no = isset($hos_no) ? $hos_no : '';

	header("Location: index.php?claims={$hos_no}&A=/$from/$to");
	exit;
}
...
if (isset($_POST['edit_update_price'])) {
	...
	$pay = (float)$_POST['pay'];
	$original_claim = (float)$_POST['claim'];
	$qty = (int)$_POST['qty'];

	if ($original_claim == 0 && $pay > 0) {
		$pay_mode = 'cash';
	} elseif ($original_claim > 0 && $pay > 0) {
		$pay_mode = 'cash';
	} elseif ($original_claim > 0 && $pay == 0) {
		$pay_mode = 'claim';
	}

	// Prepare the update query safely
	$stmt = $db->prepare('UPDATE patient_ap_services 
		SET 
			claim_amt = :claim_amt, 
			pay = :pay, 
	...
	// Redirect safely (but make sure $from and $to are set or passed)
	if (isset($_POST['from']) && isset($_POST['to'])) {
		$from = $_POST['from'];
		$to = $_POST['to'];
		header("Location: index.php?claims=$hospital_no&A=/$from/$to");
		exit;
	} else {
		header("Location: index.php?claims=$hospital_no");
		exit;
	}
}


if (isset($_POST['edit_update_date'])) {

	$stmt = $db->prepare('UPDATE patient_ap_services 
	SET date_entry = :date_entry 
	WHERE hospital_no = :hospital_no AND sn = :sn');

	$stmt->bindParam(':date_entry', $_POST['date_entry']);
	$stmt->bindParam(':hospital_no', $_POST['emr']);
	...
	<div class="col-lg-12">
		<div class="ibox float-e-margins">
			...
			<div class="ibox-content">
				<div align="center">
					<h3>FEE FOR SERVICE CLAIMS/SPECIAL PACKAGE REPORT </h3>
					...
					<table width="100%">
						<tr>
							<td width="15%">
								...
								<strong style="font-size:20px"><br><?php echo number_format($grand_amt_hmo, 2, '.', ','); ?></strong>
							</td>
							<td width="15%">
								<strong>Amount Paid: </strong>
								<strong style="font-size:20px"><br><?php echo number_format($grand_pay_amt, 2, '.', ','); ?></strong>
							</td>
							<td width="15%">
								<strong>Credits: </strong>
								<strong style="font-size:20px"><br><?php echo number_format($total_cr_pay, 2, '.', ','); ?></strong>
							</td>
							<td width="15%">
								<strong>Invoiced: </strong>
								<strong style="font-size:20px"><br><?php echo number_format($inv_processed_pay, 2, '.', ','); ?></strong>
							</td>
							<td width="15%">
								<strong>Validated: </strong>
								<strong style="font-size:20px"><br><?php echo number_format($valid_amt, 2, '.', ','); ?></strong>
							</td>

							<td width="15%">
								<strong>Un/Validated: </strong>
								<strong style="font-size:20px"><br><?php echo number_format($total_cr_claim, 2, '.', ','); ?></strong>
							</td>
						...
						<td align="right">
							<div align="right">
								...
								if (!empty($from_where)):
									...
									<?php endif; ?>
								<?php else: ?>
									<a href="index.php?ptm=all/<?= urlencode($hos_no); ?>" style="font-size:20px;">
...
<div class="modal inmodal fade" id="auth_modal" tabindex="-1" role="dialog" aria-hidden="true" data-keyboard="false" data-backdrop="static">
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
				<h4 class="modal-title" id="">View Claims Report</h4>
			</div>

			<div class="modal-body" id="auth_body">

				<form action="index.php?claims=<?php echo $hos_no . '&A=' . "/$from/$to . $pk"; ?>" method="post">


					<div class="form_sep">
						<strong>Booked By: <?php echo $checkin_by; ?></strong><br>
						<strong>Date Booked: <?php echo date("d-m-Y", strtotime($from));; ?></strong>
					</div>
					<div class="form_sep">
						<?php
						...
						<input type="text" id="auth_code" name="auth_code" maxlength="100" class="form-control" value="<?php if ($rxw["auth_code"] != '') {
																															echo $rxw["auth_code"];
																														} else {
																															echo '';
																														} ?>" required>
					...
					<div class="form_sep">
						<button type="submit" class="btn btn-success btn btn-sm" name="update_auth" id="update_auth">Update Authorization Code</button>
						<input type="hidden" name="hos_no" value="<?php echo $hos_no; ?>" />
			...
			<div class="modal-body" id="claims_body">
				...
				<table class="table table-striped table-bordered table-hover">
					...
					<tbody>
						...
						// Step 1: Fetch all notes within range
						$sql = "SELECT notes_type, notes, prepared_by, DATE(date_entry) as note_date 
								FROM notes 
								WHERE hospital_no = :hos_no AND DATE(date_entry) BETWEEN :from AND :to 
								ORDER BY date_entry";
						...
						if (count($notesFetched) > 0) {
							...
							// Step 2: Fetch all patient_ap_services in same date range for lab/pharmacy/radiology
							$sql2 = "SELECT DATE(date_entry) as serv_date, item_services, cat_type, remarks, serv_group 
									 FROM patient_ap_services 
							...
							foreach ($apResults as $rowx) {
								...
								if (!isset($apServicesByDate[$date])) {
									$apServicesByDate[$date] = ['pharmacy' => [], 'lab' => []];
								}

								if ($rowx['serv_group'] === 'Pharmacy') {
									$apServicesByDate[$date]['pharmacy'][] = htmlspecialchars($rowx['item_services']) .
										' <strong>Prescription:</strong> ' . htmlspecialchars($rowx['remarks']);
								} else {
									$apServicesByDate[$date]['lab'][] = htmlspecialchars($rowx['item_services']);
								}
							}

							// Step 3: Render table rows for each date with notes
							foreach ($notesData as $date => $noteSet) {
								...
								echo "<tr>
									<td>" . date('d /<br>M,y', strtotime($date)) . "</td>
									<td>{$noteSet['c']}</td>
									<td>{$noteSet['d']}</td>
									<td>{$noteSet['other']}</td>
									<td>";
								...
								if ($noteSet['plan'] !== '') {
									echo "<strong>Plan:</strong><br>{$noteSet['plan']}<br>";
								}
								if ($pharmacyOutput !== '') {
									echo "<strong>Medications:</strong><br>{$pharmacyOutput}<br>";
								}
								if ($labOutput !== '') {
									echo "<strong>Investigations:</strong><br>{$labOutput}";
								}
...
<div class="modal inmodal fade" id="claims_modal" tabindex="-1" role="dialog" aria-hidden="true" data-keyboard="false" data-backdrop="static">
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
			...
			<div class="modal-body" id="claims_body">
				...
				<form action="index.php?show_claims_change=<?php echo $hos_no; ?>" method="post">
					...
					<div class="form_sep">
						<button type="submit" class="btn btn-success btn btn-sm" name="show_claims_change">Show Records</button>
						<input type="hidden" name="getvalue" value="<?php echo 'all/' . $hos_no; ?>" />
						<input type="hidden" name="hospital_emr_numb" value="<?php echo $hos_no; ?>" />
					</div>
				</form>
...
<div class="modal inmodal fade" id="modal-form" tabindex="-1" role="dialog" aria-hidden="true" data-keyboard="false" data-backdrop="static">
			...
			<div class="modal-body">
				<div class="row">
					<div class="col-sm-6 b-r">
						<h3 class="m-t-none m-b">Pharmacy Prescription</h3>
					...
					<div class="col-sm-6">
						<h3>Investigation(s)</h3>
						<p><?= $investigation_desc; ?></p>
					</div>
				</div>
			</div>
		</div>
