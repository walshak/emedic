...
$totalRows_rscheckco = mysqli_num_rows($rscheckco);
 ...
 //$exst_session=$row_rscheckco['level_ses'];
		...
		$email=$row_rscheckco['email'];
		$phone=$row_rscheckco['phone'];
...
<form action="goto_interwish_pg.php" form method="post" enctype="multipart/form-data" >
  ...
  <table class="altrowstable" id="alternatecolor" width="583" border="1">
    <tbody>
      ...
      <tr>
        <td colspan="2" bgcolor="#FF6600" class="fieldname"><div align="center" class="style4">Are you sure you want to submit these Payment Data to FUT Minna?</div></td>
      </tr>
      <tr>
  ...
  <input type="hidden" name="payerEmail" id="payerEmail" value="<?php if ($email==''){echo 'info@futminna.edu.ng';}else{echo $email;} ?>" />
            <input type="hidden" name="payerPhone" id="payerPhone" value="<?php echo $phone; ?>" />
      ...
      <tr>
        ...
        <td><?php //if($student_status=='PGReturn'){ ?>
        
        <input class="btn primary" name="SUBMIT" value="Submit Payment" onClick="ValidateForm(this.form)" type="submit" />
        	<?php // } else {
				//echo '2015/2016 Registration Closed';
			//}
				 ?>
        </td>
      </tr>
