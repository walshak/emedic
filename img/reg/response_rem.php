...
<body>
		...
		<?php
				//echo $orderID;
				$setdate=date("Y-m-d");
		$updateSQL = sprintf("UPDATE payment_log SET PaymentState=%s, ResponseDescription=%s, PaymentDate=%s, RRR=%s WHERE PaymentID=%s",
		...
		?>    
            ...
            $query_rst_edata = sprintf("SELECT * FROM pgapplication WHERE applicant_id= %s", GetSQLValueString($stdid, "text"));
            $rst_edata =  mysqli_query($conn,$query_rst_edata) or die(mysqli_connect_errno());
            $row_rst_edata = mysqli_fetch_array($rst_edata);
        ...
        <td colspan="2" class="fieldname">&nbsp;</td>
            ...
            <a href="javascript:window.print()"><button  class="btn primary">
                    ...
                    <?php }	else{ ?>
                                    ...
                                    <?php } ?> 
                                      <p><b>Reason: </b><?php echo $response_message; echo $orderId; ?><p>
                     <?php }?>
