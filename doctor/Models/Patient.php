...
class Patient
{
    ...
    public function getByHospitalNo($hospital_no){
        ...
        if(!empty($enrolee)){
            $sql = "SELECT insurance_no,insurance_name, interest,insurance_type,services_access,payment_mode,add_minus FROM insurance_tbl WHERE insurance_no = ? LIMIT 1";
            $stmt = $this->dbCon->prepare($sql);
            ...
            if(!empty($enrolee)){
                ...
                $enrolee->add_minus = $hmo_no_info->add_minus;
