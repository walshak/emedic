...
class Patient
{
    ...
    public function getByHospitalNo($hospital_no){
        ...
        if(!empty($enrolee)){
            $sql = "SELECT insurance_name, interest,insurance_type,services_access,payment_mode,insurance_no FROM insurance_tbl WHERE insurance_no = ? LIMIT 1";
            $stmt = $this->dbCon->prepare($sql);
    ...
    public function getByHospitalNo2($hospital_no){
        $sql = "SELECT e.*,i.insurance_name,i.interest,i.insurance_type,i.insurance_no FROM enrollee as e INNER JOIN insurance_tbl as i ON e.hmo_no=i.insurance_no WHERE hospital_no = ? ";
        $stmt = $this->dbCon->prepare($sql);
        $stmt->execute([$hospital_no]);
    ...
    private function fetch($arr = [], $all = false)
    {
        ...
        $sql = "SELECT e.*,i.insurance_name,i.interest,i.insurance_type,i.insurance_no FROM enrollee as e INNER JOIN insurance_tbl as i ON e.hmo_no=i.insurance_no WHERE 1 $where_clause ";
        $stmt = $this->dbCon->prepare($sql);
        $stmt->execute();
