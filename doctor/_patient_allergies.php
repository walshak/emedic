...
if (isset($_POST['hospital_no'])) {
    ...
    if ($stmt->rowCount() == 0) {
        ...
        $update_allergy_stmt = $db->prepare('UPDATE c_d_remarks SET complain = ?, updated_at =?, prepared_by = ? WHERE sn=?');
        $update = $update_allergy_stmt->execute(array($allergies_notes, $now_setdate, $_SESSION['fullname'], $allergies_row["sn"]));
...
if (isset($_POST['update_allergies_btn'])) {
    ...
    $update_allergy_stmt = $db->prepare('UPDATE c_d_remarks SET complain = ?, updated_at = ?, updated_by = ? WHERE sn=?');
    $update = $update_allergy_stmt->execute(array($allergies_notes, $now_setdate, $_SESSION['id'], $allergy_sn));
