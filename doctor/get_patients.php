...
try {
    ...
    } elseif ($mode === 'all_patients_dailysis') {
        $query = "
        SELECT DISTINCT hospital_no, patient_name
            FROM dialysis WHERE hospital_no LIKE :term OR patient_name LIKE :term2
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        if ($mode === 'external') {
            ...
            $name = $is_vip ? 'VIP: ' . $row["hospital_no"] : $row["hospital_no"] . ' - ' . $row["patient_name"];
            $results[] = array(
                'id'   => $row['hospital_no'],
                'text' => $name
            );
