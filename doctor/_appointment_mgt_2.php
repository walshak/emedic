...
<script>
	...
	let autosaveTimer;
	const saveButton = document.getElementById("save-mgt-button");
	const saveButton2 = document.getElementById("save-mgt-button_physio");
	const editor = $('#mgt_notes');
	const autosaveStatus = $('#autosave-status');
	const autosavingStatus = $('#autosaving-status');
	var autosaveReatored = false;
	...
	// Function to autosave the content
	function autoSave() {
		const hospitalNo = editor.data('hospital_no');
		const appNo = editor.data('app_no');
		const doctor = editor.data('doctor');
		const content = getEditorContent();


		//no need to call server if content has not changed.
		if (lastAutosaveText == content) {
			return 0;
		}

		// Show "Autosaving..." status
		autosavingStatus.show();

		//only perform autosave after the user has gotten a chance to chose if they wish to restore the old one or not.
		if (autosaveReatored == false) {
			return 0;
		}

		// Send the content to the server
		$.ajax({
			url: '_autosave.php', // URL to handle autosave
			type: 'POST',
			data: {
			...
			success: function(response) {
				// Hide "Autosaving..." status and show "Autosave Enabled"
				autosavingStatus.hide();
				autosaveStatus.show();
				lastAutosaveText = content;
			},
			error: function() {
				// Hide "Autosaving..." status if there's an error
				autosavingStatus.hide();
			}
		});
	}

	// Function to restore autosave content on page load
	function restoreAutosave() {
		...
		if (hospitalNo != '' && appNo != '') {
			$.ajax({
				url: '_autosave.php', // URL to fetch the latest autosave data
				type: 'GET',
				data: {
					hospital_no: hospitalNo,
					app_no: appNo,
				...
				success: function(response) {
					if (response.content != "") {
						v = confirm('An Autosave exists for this appointment, do you want to restore it? ');
						if (response && response.content && v) {
							editor.html(response.content); // Restore content to editor
							autosaveReatored = true;
						} else {
							autosaveReatored = true;
						}
					} else {
				...
				error: function() {
					console.log("Error loading autosave data.");
					autosaveReatored = true;
				}
			});
		}
	...
	// Function to handle manual save button click
	function handleSave() {
		// Remove autosave data when manual save is performed
		$.ajax({
			url: '_autosave.php', // URL to delete the saved autosave data
			type: 'POST',
			data: {
				hospital_no: hospitalNo,
				app_no: appNo,
			...
			success: function(response) {
				console.log("Autosave deleted after manual save.");
			}
		});
	}

	//trigger autosave restoration when my user focuses on the inpu field
	...
	// Clear autosave interval when user clicks to save manually
	if (saveButton) {
		saveButton.addEventListener('click', handleSave);
	}
	if (saveButton2) {
		saveButton2.addEventListener('click', handleSave);
	}
