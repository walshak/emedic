...
if (isset($_POST['update-note'])) {
    ...
    if (isset($_POST['pro_note'])) {
        $notes =  $_POST['pro_note'];
        $count_update_call =  $_POST['count_update_call'];
        $id =  $_POST['id'];
        
        if($count_update_call > 0 ){
            if (!empty($notes)) {

                $update = editNotes($db, $id, $notes);
    ...
    echo json_encode(["status" => 200, "data" => $data, "message" => $message, "id" => null]);
    exit;
}
