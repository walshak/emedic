...
if (isset($_GET["del"])) {
	$del = $_GET['del'];
	$stmt = $db->prepare("DELETE FROM hrlvapply WHERE sn = :del");
...
if (isset($_POST["add_leave"])) {
	...
	if ($t_days <= $req_days) {
		...
		if ($apply_type == 'Yearly') {
			$yr = date("Y");
			$stmt = $db->prepare("SELECT * FROM hrlvapply WHERE year = :year AND type_leave = :type_leave AND status = 'finish' AND ECode = :EmployeeCode");
			$stmt->bindParam(':year', $yr, PDO::PARAM_INT);
			$stmt->bindParam(':type_leave', $leave_type, PDO::PARAM_STR);
			$stmt->bindParam(':EmployeeCode', $EmployeeCode, PDO::PARAM_STR);
			$stmt->execute();

			$stmt2 = $db->prepare("SELECT * FROM hrlvapply WHERE year = :year AND type_leave = :type_leave AND status = 'pending' AND ECode = :EmployeeCode");
			$stmt2->bindParam(':year', $yr, PDO::PARAM_INT);
			$stmt2->bindParam(':type_leave', $leave_type, PDO::PARAM_STR);
			...
			$stmt = $db->prepare("SELECT * FROM hrlvapply WHERE year = :year AND month = :month AND type_leave = :type_leave AND status = 'finish' AND ECode = :EmployeeCode");
			$stmt->bindParam(':year', $yr, PDO::PARAM_INT);
			...
			$stmt2 = $db->prepare("SELECT * FROM hrlvapply WHERE year = :year AND month = :month AND type_leave = :type_leave AND status = 'pending' AND ECode = :EmployeeCode");
			$stmt2->bindParam(':year', $yr, PDO::PARAM_INT);
...
function Add_leave($leave_type, $approvers_list)
{
	...
	$Ecode = $part[0];
	$name = $part[1];
	$stmt = $db->prepare("INSERT INTO hrlvapply(ECode, Name, dept_id, approvers_list, date_apply, starting_date, ending, month, year, reason, type_leave, days, status, backup_staff) 
	...
	$stmt->bindParam(':approvers_list', $approvers_list, PDO::PARAM_STR);
	$stmt->bindParam(':date_apply', $setdate, PDO::PARAM_STR);
	$stmt->bindParam(':starting_date', $_POST["start"], PDO::PARAM_STR);
	$stmt->bindParam(':ending', date('y-m-d', strtotime($_POST["start"] . '+' . $_POST["t_days"] . ' days')), PDO::PARAM_STR);
	...
	$stmt->bindParam(':status', $status = 'pending', PDO::PARAM_STR);
	$stmt->bindParam(':backup_staff', $backup_staff, PDO::PARAM_STR);
			...
			<div class="ibox-content">
				...
				<script>
					function do_leave_calculation(ECode = null) {
						...
						var ECode = document.getElementById('employ_code').value;
