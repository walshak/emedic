...
function Add_leave($leave_type)
{
	...
	$update = $db->prepare("INSERT INTO hrlvapply (ECode, date_apply, starting_date, month, year, reason, type_leave, days, status) 
	VALUES (:ECode, :date_apply, :starting_date, :month, :year, :reason, :type_leave, :days, :status)");

	// Bind the parameters with their corresponding values
	$update->bindParam(':ECode', $_POST["EmployeeCode"], PDO::PARAM_STR);
...
<div class="row">
						...
						<div class="col-lg-4">
							...
							<div id="form_sep">
								...
								<select name="EmpCode" class="input-sm chosen-select" style="width:350px;">
									...
									<?php $stmt = $db->query("SELECT DISTINCT emp.EmployeeCode,emp.FirstName,emp.LastName FROM hremp as emp inner join hrlvapply as app on emp.EmployeeCode=app.ECode WHERE emp.status=0 order by emp.FirstName");
	...
	<div class="col-lg-12">
		<div class="ibox float-e-margins">
			<div class="ibox-title">
				<h5>Staff Leave Pending & Approval Table</h5>
			</div>
			...
			<div class="ibox-content">
				...
				if (isset($_POST['apply_apply'])) {
					if (isset($_POST['leave_type']) and $_POST['leave_type'] != '') {
						$leave_type = $_POST['leave_type'];
						$stmt = $db->query("SELECT * FROM hrlvapply where status='$leave_type' and (dept_id='$dept_id' or approvers_list LIKE '%$Designation%') order by sn desc");
					} elseif (isset($_POST['EmpCode']) and $_POST['EmpCode'] != '') {
						$EmpCode = $_POST['EmpCode'];
						$stmt = $db->query("SELECT * FROM hrlvapply where ECode='$EmpCode' and (dept_id='$dept_id' or approvers_list LIKE '%$Designation%') order by sn desc");
					}
				} else {
					$stmt = $db->query("SELECT * FROM hrlvapply where status='pending' and (dept_id='$dept_id' or approvers_list LIKE '%$Designation%') order by sn desc");
				}
					...
					<table class="table table-striped table-bordered table-hover">
						...
						<tbody>
							...
							?>
								...
								<tr>
									<td><?php echo $row['ECode'] . ':<br> ' . $row['Name']; ?></td>
									...
									<td><?php
										...
										//	echo $ddate = strtotime($row['starting_date']);
										...
										if ($row['status'] == 'Approve') {
											...
											$date1 = new DateTime($main_date);
											$date2 = new DateTime($resumpdate2);
										...
										<?php } else { ?>
											...
											<div class="btn-group">
												...
												<ul class="dropdown-menu">
													...
													<li> <a onclick="printLeaveRequest(<?php echo $row['sn']; ?>)">Print</a> </li>
...
<script>
	function printLeaveRequest(sn) {
		// Open the print window with the leave details
		window.open('print_leave_req.php?sn=' + sn, '_blank');
	}
</script>