...
try {
    ...
    $gl_acct = isset($_POST['gl_acct']) ? $_POST['gl_acct'] : ''; //comma separated string of
    ...
    if (!empty($gl_acct)) {
        $openingSql .= " AND account_no IN ($gl_acct)";
    ...
    // Prepare the closing balance query with filters
    $closingSql = "SELECT dr_amt, cr_amt 
                   FROM chart_ledger 
                   WHERE DATE(date_entry2) >= ? AND DATE(date_entry2) <= ?";
    $closingParams = array($start_date, $end_date);

    // Add conditions dynamically
    if (!empty($prep)) {
        $closingSql .= " AND prepared_by = ?";
        $closingParams[] = $prep;
    }
    if (!empty($gl_acct)) {
        $closingSql .= " AND account_no IN ($gl_acct)";
        // $closingParams[] = $gl_acct;
    }
    if (!empty($DR_CR) && ($DR_CR == 'CREDIT' || $DR_CR == 'DEBIT')) {
        $closingSql .= " AND transc_type = ?";
        $closingParams[] = $DR_CR;
    }
    if (!empty($account_payable)) {
        $closingSql .= " AND insurance_no = ? AND account_no = 2124";
        $closingParams[] = $account_payable;
    }
    if (!empty($hmo_dues)) {
        $closingSql .= " AND insurance_no = ? AND account_no = 1114";
        $closingParams[] = $hmo_dues;
    }
    if (!empty($account_recievables)) {
        $closingSql .= " AND hospital_no = ? AND account_no = 1114";
        $closingParams[] = $account_recievables;
    }
    if (!empty($patient_deposit)) {
        $closingSql .= " AND hospital_no = ? AND account_no = 2121";
        $closingParams[] = $patient_deposit;
    }

    // Execute the closing balance query
    $closingStmt = $db->prepare($closingSql);
    $closingStmt->execute($closingParams);

    // Fetch closing balances one by one
    $closingDR = 0;
    $closingCR = 0;
    while ($entry = $closingStmt->fetch(PDO::FETCH_ASSOC)) {
        $closingDR += isset($entry['dr_amt']) ? $entry['dr_amt'] : 0;
        $closingCR += isset($entry['cr_amt']) ? $entry['cr_amt'] : 0;
    }
    ...
    while ($entry = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        $account_label = '';
        if ($entry['account_no'] == 2124) {
            $account_label = '[' . $entry['account_no'] . '] ' . getInsuranceName($entry['insurance_no']);
        } else {
            $account_label = '[' . $entry['account_no'] . '] ' . getAccountName($entry['account_no']);
        }

        $data[] = array(
            'sn' => $sn,
            'account_no' => $account_label,
            'item_services' => $entry['item_services'],
            'client' => $entry['client'],
            ...
            'lg_ref_no' => $entry['ref'], // for grouping
        );
    ...
    echo json_encode(array(
        ...
        'closing_totals' => array(
            'closingDR' => number_format($closingDR + $openingDR, 2),
            'closingCR' => number_format($closingCR + $openingCR, 2),
            'closingBalance' => number_format($closingBalance + $openingBalance, 2)
        )
    ));
} catch (Exception $e) {
    logError("Error: " . $e->getMessage());
    error_log($e->getMessage());
    echo json_encode(array("error" => "An error occurred while fetching data.", "hh"=> $e->getMessage()));
}
