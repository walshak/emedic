	...
	try {
		...
		for ($i = 0; $i < count($accounts); ++$i) {
			...
			if (strpos($account, "account_payable") !== false) {
				...
				$hospital_no = null;
			} elseif (strpos($account, "patient_deposit_walkin_") !== false) {
				// Handle walk-in patient deposits - use transc_code as hospital_no
				$hospital_no = str_replace("patient_deposit_walkin_", "", $account);
				$account = 2121;
				$supplier_id = 1000; // Default HMO for walk-in patients
			} elseif (strpos($account, "patient_deposit") !== false) {
				// Handle regular patient deposits
	...
	<div id="wrapper">
												...
												<td width="40%">
													...
													?>
														<div class="search-container">
															<input type="text" id="patient-search-input" class="form-control" placeholder="Search Patient..." autocomplete="off" required>
															<div id="account_checker_deposit" class="patient-list"></div>
															<input type="hidden" name="account[]" id="account_checker_deposit_input" class="form-control account" required>
														</div>
													<?php } else {
														$patient_deposit = 0; ?>
		...
		<script>
			function fetchPatients(searchTerm) {
				$.ajax({
					...
					success: function(data) {
						const $dropdown = $('#account_checker_deposit');
						$dropdown.empty();

						if (data.length > 0) {
							data.forEach(function(patient) {
								const patientType = patient.patient_type || 'regular';
								const dataAttribute = patientType === 'walk_in' ? 'data-transc-code' : 'data-hospital-no';
								const patientId = patient.patient_id;

								$dropdown.append(`
                                <div class="patient-item" ${dataAttribute}="${patientId}" data-patient-type="${patientType}">
                                    <strong>${patientId}</strong>: ${patient.surname} ${patient.fname} ${patient.oname}
                                    <br><small>Balance: ${patient.balance}</small> <span class="badge ${patientType === 'walk_in' ? 'badge-info' : 'badge-primary'}">${patientType === 'walk_in' ? 'Walk-in' : 'Regular'}</span>
                                </div>
                            `);
							});
						} else {
							$dropdown.append('<div class="no-results">No patients found</div>');
						}
			...
			$(document).ready(function() {
				...
				const $dropdown = $('#account_checker_deposit');
				const $accountInput = $('#account_checker_deposit_input');
				...
				$(document).on('click', '.patient-item', function() {
					...
					const patientName = $(this).text().split(':')[1].trim().split('Balance')[0].trim();

					if (patientType === 'walk_in') {
						const transcCode = $(this).data('transc-code');
						$searchInput.val(patientName);
						$accountInput.val('patient_deposit_walkin_' + transcCode);
					} else {
						const hospitalNo = $(this).data('hospital-no');
						$searchInput.val(patientName);
						$accountInput.val('patient_deposit' + hospitalNo);
					}

					$dropdown.hide();
					call_bal3(); //get the balance of the selected patient
				});
		...
		<script>
			...
			function call_bal3() {
				var account_checker_deposit = document.getElementById("account_checker_deposit_input").value;
				...
				$.ajax({
					...
					success: function(data) {
						...
						var json = JSON.parse(data);
						//if it is a negative balance, set it in the cr column else set it in dr col
						if (parseFloat(json["amount"]) > 0) {
							document.getElementById("amount_dr").value = json["amount"];
							document.getElementById("amount_cr").value = 0;

						} else {
							document.getElementById("amount_dr").value = 0;
							document.getElementById("amount_cr").value = parseFloat(json["amount"]) * -1;
						}
