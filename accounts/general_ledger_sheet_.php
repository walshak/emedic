...
// Get date range from GET or default to today
...
function getLedgerEntries($db, $account_code, $start, $end)
{
    $stmt = $db->prepare('
        ...
        WHERE cl.account_no = ? AND DATE(cl.date_entry2) BETWEEN ? AND ?
...
function getAllTotals($db, $start, $end)
{
    $stmt = $db->prepare('
        ...
        FROM chart_ledger cl
        WHERE DATE(cl.date_entry2) BETWEEN ? AND ?
    ');
...
function getClassTotals($db, $class_id, $start, $end)
{
    $stmt = $db->prepare('
        ...
        WHERE cg.class_id = ? AND DATE(cl.date_entry2) BETWEEN ? AND ?
    ');
...
function getGroupTotals($db, $group_id, $start, $end)
{
    $stmt = $db->prepare('
        ...
        FROM chart_ledger cl
        JOIN chart_accounts ca ON cl.account_no = ca.account_code
        WHERE ca.account_group = ? AND DATE(cl.date_entry2) BETWEEN ? AND ?
    ');
...
// Helper to get totals for an account
                ...
                <div class="col-lg-12">
                    <div class="ibox float-e-margins">
                        ...
                        <div class="ibox-content">
                            ...
                            <?php
                            // Calculate main totals by summing all account totals in the visible period
                            $main_total_dr = 0;
                            $main_total_cr = 0;
                            foreach ($classes as $class) {
                                ...
                                foreach ($class_groups as $group) {
                                    ...
                                    foreach ($group_accounts as $account) {
                                        ...
                                        $main_total_dr += $acc_totals['dr'];
                                        $main_total_cr += $acc_totals['cr'];
                                    }
                                }
                            }
