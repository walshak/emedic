...
if (isset($_POST['load_table_items'])) {
	...
	if ($result >= 1) {
		...
		if ($oncredit == 1) {
			...
			$stmt_ = $db->prepare("SELECT 1 FROM admission WHERE (adm_status = 3 OR adm_status IS NULL) AND hospital_no = ? ORDER BY sn DESC LIMIT 1");
			$stmt_->execute([$hosp_no]);
			if ($stmt_->rowCount() > 0) {
				$inpatient_credit_limit = $_SESSION['inpatient_credit_limit'];
				...
				$inpatient_credit_limit = 0;
			}
		}

		$bal_items = call_current_balance($db, $hosp_no, $inpatient_credit_limit);
		...
		for ($x = 0; $x < $result; $x++) {
			...
			if ($lab_combo == 1) {
				$cb_no = $test_id;
				$stmt2 = $db->prepare("SELECT c.test_id, l.test, l.category, l.dept FROM lab_combos_items AS c 
...
VALUES (:app_no,:labrequest_no,:patient,:patient_name,:test_id,:test_name,:lab_cat,
			...
			:requesting_physician,:lab_combo_request_no)");
					...
					$update = $db->prepare("UPDATE lab_manage SET lab_combos = 0 WHERE test_id = :test_id");
					$update->bindParam(':test_id', $test_id);
					$update->execute();
				}
			...
			<table class="table table-striped table-bordered table-hover dataTables-example">
				...
				<tbody>
					...
					foreach ($myArray as $value) {
						...
						$stmt_list = $db->prepare("SELECT  result_date, request_date, approved_by, request_by 
	FROM lab_manage 
	WHERE labrequest_no = :labrequest_no AND app_no = :app_no");
						...
						if ($stmt_list->rowCount() > 0) {
							...
							///echo $data_capture_status = $row['data_capture_status'];
					...
					?>
						<tr>
							...
							<td><?php if ($lab_combo == 1) {
								...
								if ($data_capture_status == 'queue' || $data_capture_status == 'specimen') {
									echo '<strong>[ ' . ucfirst($data_capture_status) . ' ]</strong>';
									$button_title = 'New Result';
									$button_color = 'success';
									$test_status = '0';
								} else {
									echo '<strong>[ ' . ($data_capture_status == 'result' ? 'Approve Pending' : 'Approved') . ' ]</strong>';
									$button_title = 'Edit';
							...
							<td>
								<input type="button" name="edit" value="Notes" data-target="#modal" id="<?= $labrequest_no ?>" class="btn btn-success btn-xs view_notes" <?= ($_SESSION['speciality'] == 'Administrator' || $_SESSION['speciality'] == 'Receptionist') ? 'disabled' : '' ?>>
							</td>
							<td>
		...
		WHERE drug_sn = :lab_combo_request_no");
								...
								if ($_SESSION['payfrom_status'] == 1) {

									$stmt = $db->prepare("SELECT 
								SUM(dr_amt) AS TOTAL_DEBITS, 
								SUM(cr_amt) AS TOTAL_CREDITS
							FROM chart_ledger 
							WHERE hospital_no = :hospital_no AND account_no = '2121'");
									...
									if ($stmt->rowCount() > 0) {
										...
										$current_balance = $row['TOTAL_CREDITS'] - $row['TOTAL_DEBITS'];
									}
								}
								...
								<?php } else {
									...
									if ($enable_cr_post == 1) {
										...
										$pay_lock = $data_capture_status == 'queue' ? 1 : 0;
									} else {
										$b_title = 'Not Paid';
										$pay_lock = $data_capture_status == 'queue' ? 1 : 0;
									}
								...
								if ($data_capture_status == 'approve' and $day > $grace) {
									$pay_lock = 1;
									$view_status = 'yes';
									$edit_status = " <br>[Exceed:" . $grace . " days]";
								} elseif ($interface == 'print' and $data_capture_status == 'result') {
									$pay_lock = 1;
							...
							<td>
								...
								?>
									<input type="button" name="edit" value="<?= $button_title; ?>" <?php if ($pay_lock == 1) { ?>disabled <?php } ?> onClick=" show_result_sheet('<?php echo $_detail . '__' . $paystatus . '__' . $cr . '__' . $test_status . '__' . $lab_combo_request_no; ?>')" class="btn btn-<?= $button_color; ?> btn-xs" />
								<?php } ?>
