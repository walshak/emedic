...
if ($_POST["MM_update"] == 'insert_form_fields') {
    ...
    $sub = lab_scan_fields($test_id, $test_id, $test_name);
}
...
if($_POST['buz']=='IN'){
	...
	/// GET BILLING PARAMETERS AND INSURANCE DETAILS 	
	...
	/// admission status
	$stmt_adm = $db->query("SELECT app_no FROM admission WHERE adm_status = '3' AND hospital_no = :patient AND date_discharge = '' ORDER BY sn DESC LIMIT 1");
	$stmt_adm->bindParam(':patient', $patient, PDO::PARAM_STR);
	$stmt_adm->execute();
	...
	if ($stmt_adm->rowCount() > 0) {
		...
		$adm_status = 0;
							...
							$stmt = $db->prepare("SELECT * FROM lab_manage WHERE patient = :patient AND DATE(request_date) = :request_date");
							...
							if ($stmt->rowCount() > 0 && $adm_status == '0') {
	...
	if (isset($_POST["specimen"]) and $_POST["specimen"]!=''){ 
	   			$specimen="";
					foreach ($_POST["specimen"] as $Aa){
						$specimen.=$Aa .', ';
					}
			}else{
				$specimen="";	
	}
				$all_specimen=substr($specimen, 0, -2);
	
	/// LAB COMBINATION TESTS HERE
	
		if (isset($_POST["labcombos"]) and $_POST["labcombos"]!=''){ 
		
				foreach ($_POST["labcombos"] as $combos_id){
					
					//$stmt=$db->query("SELECT test_id FROM lab_combos_items WHERE combos_id='$combos_id'");
						//	while($rowC = $stmt->fetch(PDO::FETCH_ASSOC)){
										$test_id=$rowC['test_id'];
								$stmt2=$db->query("SELECT * FROM lab_scan WHERE sn='$combos_id'");
							if($stmt2->rowCount()>0){									
									$roww= $stmt2->fetch(PDO::FETCH_ASSOC);
        $values[]=$roww["sn"] .'__'. $roww["test"] .'__'. $roww["dept"].'__'.$roww["hosp_price"].'__'.$roww["nhis_price"].'__'.$roww["coverage"].'__'.$roww["insurance_type"].'__'.$roww["category"];
									}
								/// 
								
						//	}
			}
			/// end of combos looping
				$combo_test=1;
	$sub_lab=lab_request_process($values,$patient,$patient_name,$group_id,$all_specimen,$setdate,$sn,$section,$combo_test,$insurance,$interest,$insurance_no,$add_minus);
}
...
$stmt=$db->query("SELECT sn FROM lab_manage order by sn DESC limit 1");
					if($stmt->rowCount()==0){
						$sn=1;
					}else{
				$roww=$stmt->fetch(PDO::FETCH_ASSOC);
	...
	foreach ($values as $a){
			$lab_request=$a;
				...
				if($_POST['buz']=='IN'){
					...
					$setdate2=date("Y-m-d");
					$queue = "queue";
				$lab_reqno = $code . date('d').$patient. sprintf('%08d', $sn);
...
$stmt->execute();
		...
		$stmt2->bindParam(':business_service_center', $_POST['buz'], PDO::PARAM_STR);
		$stmt2->bindParam(':referral', $_POST["Referred"], PDO::PARAM_STR);
	...
	$stmt->bindValue(':transact_date', $empty, PDO::PARAM_STR);
...
function lab_request_process($values,$patient,$patient_name,$group_id,$all_specimen,$setdate,$sn,$section,$combo_test,$insurance,$interest,$insurance_no,$add_minus){
...
foreach ($values as $a){
	...
	$stmt = $db->prepare("SELECT * FROM lab_combos_items WHERE test_id = :lab_request AND combos_id = :com_id");
	$stmt->bindParam(':lab_request', $lab_request, PDO::PARAM_STR);
	$stmt->bindParam(':com_id', $_POST["com_id"], PDO::PARAM_STR);
	$stmt->execute();
	
	if($stmt->rowCount()==0){
		$com_id = $_POST["com_id"];
		$lab_request = $_POST["lab_request"];
		
		$stmt = $db->prepare("INSERT INTO lab_combos_items(combos_id, test_id) VALUES (:com_id, :lab_request)");
...
if($_POST["MM_update"]=='approve_results_2'){
	  ...
	  if(isset($_POST["abnormal"])){$ab=1;}else{$ab=0;}
...
// Prepare the SQL statement with placeholders
$stmt = $db->prepare("INSERT INTO pharm_ext(sn, transc_code, cust_name, description, gender, dob, phone, address, email_address, referral, date_ap, captured_by) VALUES (:sn, :transc_code, :cust_name, :description, :gender, :dob, :phone, :address, :email_address, :referral, :date_ap, :captured_by)");
...
$stmt->bindParam(':referral', $_POST['referral'], PDO::PARAM_STR);
...
$stmt = $db->prepare("UPDATE pharm_ext SET cust_name=:cust_name, gender=:gender, dob=:dob, phone=:phone, address=:address, email_address=:email_address, referral=:referral WHERE transc_code=:transc_code");
