...
if (isset($_POST["hospital_no"])) {
    ...
    try {
        ...
        if ($stmt_ADM->rowCount() > 0) {
            ...
            if ($chkDischarge->rowCount() > 0 || $billable === 'no') {
                $db->prepare("
                INSERT INTO admission_billable_list (hospital_no, room_bed, accom_gen_date)
                VALUES (:hospital_no, :room_bed, :today)
                ON DUPLICATE KEY UPDATE accom_gen_date = :today2")->execute([
                    ':hospital_no' => $hospital_no,
                    ':room_bed' => $room_bed,
                    ':today' => $today,
                    ':today2' => $today
            ...
            if ($today == $formatted_date_entry && $totalRows == 1) {
                ...
                if ($chk->rowCount() == 0 && !empty($refRows)) {
                    foreach ($refRows as $refRow) {
                        ...
                        if ($charge_date != $today) {
                            if ($remarks === 'auto_deduct') {
                                $remarks = 'auto_deduct2';
                            }
                        }
            ...
            if (!empty($refRows)) {
                ...
                UPDATE patient_ap_services
                SET remarks = CASE 
                    WHEN remarks = 'auto_deduct' THEN 'auto_deduct2'
                    ELSE remarks
                END
                WHERE hospital_no = :hospital_no 
                AND DATE(date_entry) = :latest_date 
