...
if (isset($_POST['save_change'])) {
	...
	$qury_services = 'SELECT item_services, qty, remarks FROM patient_ap_services WHERE sn = :sn';
	...
	if ($row_drug['hosp_price'] > 0 &&  $row_drug['cash_price'] > 0) {
		if ($bal >= $new_qty && $new_qty > 0) {
			...
			$nhis_price = $nhis_price * $new_qty;
			$hosp_price = $hosp_price * $new_qty;
			$cash_price = $cash_price * $new_qty;
			...
			if ($_SESSION['dispensory'] == 1) {
				...
				if ($stmt->rowCount() > 0) {
					...
					$cash_price = $row_drug['cash_price'] * $new_qty;
					$nhis_price = $row_drug['nhis_price'] * $new_qty;
				}
			}

			include_once '../inc/calc_drugs.php';
			...
			if ($insurance_type == 'NHIS') {
				if ($nhis_price > 0) {
					...
					$percent = $interest / 100;
					$amt_paying = $hosp_price * $percent;
					$claim_amt = $hosp_price - $amt_paying;
					$pay_mode = 'cash';
					$ccop_int_charge = $interest;
				}
			}

			$qury_price = 'SELECT price FROM hmo_stocks_tariff WHERE stock_sn = :drug_sn AND price > 0 AND (hmo = :insurance_no OR hmo = :insurance)';
			...
			if ($amt_paying > 0 or $claim_amt > 0) {
...
if (isset($_POST['ph_add_drug'])) {
	...
	if ($row_drug['hosp_price'] > 0 && $row_drug['cash_price'] > 0 && $new_qty > 0) {
		if ($bal >= $new_qty) {
			...
			$stmt = $db->prepare('SELECT price FROM hmo_stocks_tariff WHERE stock_sn = :item_sn AND price > 0 AND hmo = :insurance_no');

			$stmt->bindParam(':item_sn', $item_sn, PDO::PARAM_STR);
			$stmt->bindParam(':insurance_no', $insurance_no, PDO::PARAM_STR);
			$stmt->execute();
...
if (isset($_GET['hos_no']) or isset($_POST['in_patient']) or isset($_GET['chg'])) {
	...
	$emr = $hos_no;
	require_once '../billing/current_bal.php';
	...
	$inpatient_credit_limit = $status['inpatient_credit_limit'];

	$bal_items = call_current_balance($db, $hos_no, $inpatient_credit_limit);
	$current_balance =  $bal_items["current_balance"];
	$credit_limit =  $bal_items["credit_limit"];
	$show_status =  $bal_items["show_status"];
} else {
...
if (isset($_GET['dl'])) {
	...
	$qry_drug = $db->prepare('DELETE FROM patient_ap_services WHERE sn = :sn');
...
if (isset($_GET['cl'])) {
	...
	$stmt = $db->prepare('SELECT qty, drug_sn, claim_amt, pay FROM patient_ap_services WHERE sn = :sn');
	...
	if ($stmt->rowCount() > 0) {
		...
		// Calculate new claim_amt and pay based on qty
		$claim_amt_per_unit = $claim_amt / $qty;
		$pay_per_unit = $pay / $qty;

		// Update the record in the database
		$updateSQL = $db->prepare('UPDATE patient_ap_services SET invoice_status = :invoice_status, claim_amt = :claim_amt, pay = :pay WHERE sn = :sn');
		$invoice_status = '0'; // Assuming '0' as the invoice status value
		$updateSQL->bindParam(':invoice_status', $invoice_status);
		$updateSQL->bindParam(':claim_amt', $claim_amt_per_unit);
		$updateSQL->bindParam(':pay', $pay_per_unit);
...
<div class="row">
	...
	<div class="col-lg-7">
		<div class="ibox float-e-margins">
			<form method="post" action="index.php?<?php echo 'presc&hos_no=' . $hos_no; ?>">
				...
				<button class="btn btn-success btn-xs" type="submit" name="skip_popup_2" id="skip_popup_2" value="<?php echo $hos_no; ?>" style="font-size: 15px; color:white; ">Auto.</button
					</form>
				...
				<input type="button" name="on_cr" value="Biodata" data-target="#modal" id="<?php echo $hos_no; ?>" class="btn btn-primary btn-xs bio_data_link" style="font-size: 15px; color:white; " /> &nbsp;
				<?php if ($vip == 0) { ?>
						...
						<tr>
							<td>
								<div style="font-size:14px;">
									...
									|&nbsp;<strong style="color: saddlebrown;">Insurance's status: </strong><?php echo ' <i><b>' . $insurance_name . '[' . $insurance . ']</b></i>'; ?>
								</div>
							</td>
							<td>
								<div style="font-size:14px;">
									...
									if ($stmt->rowCount() > 0) {
										...
										while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
											$allergies .= $row['complain'] . ' : ';
										}
									?>
										<strong style="color: red;">Allergies:</strong><br>
										<?php echo $allergies; ?>
									<?php } else { ?>
	...
	<div class="col-lg-12">
		<div class="ibox float-e-margins">
			...
			<div class="ibox-content">
				...
				if (isset($_GET['dsp'])) {
					$details = 'dsp';
				} elseif (isset($_GET['dcr'])) {
					$details = 'dcr';
				} elseif (isset($_GET['hx'])) {
					$details = 'hx';
	...
	AND date_entry BETWEEN :backdated_date AND :current_date');
											...
											if ($total_qty_invoiced > 0 and $total_qty_invoiced >= $qty) {
												$val = 1;
												$disabled = 'disabled';
												$new_request = '<b>[Running low: drug invoiced for another, pending payment.]</b>';
											} elseif ($qty <= 0) {
												$val = 1;
												// $disabled ='disabled';
												$new_request = '<b>[INSUFFICIENT QUANTITY]</b>';
											} elseif ($row['pay_mode'] == 'claim' and $row['process_claim'] == '0' and $row['pay'] == '0' && $_SESSION['col5'] == 1) {
												$val = 0;
												$disabled = '';
												$new_request = '[ New Request ]';
											} elseif ($row['pay_mode'] == 'claim' and $row['process_claim'] == '0' and $row['pay'] == '0') {
												$val = 1;
												$disabled = 'disabled';
												$new_request = '[Validation Pending]';
											} else {
												$val = 0;
												$disabled = '';
												$new_request = '[ New Request ]';
											...
											<tr>
												...
												<td>
													...
													<input type="checkbox" <?php echo $disabled; ?> onClick="UpdateCost()"
														value="<?php echo $row['sn'] . '__' . $row['product_name'] . '__' . $row['pay'] . '__' . $row['claim_amt'] . '__' . $row['qty'] . '__' . $row['sale_sn'] . '__' . $insurance_no . '__' . $insurance . '__' . $row['interest'] . '__' . $row['remarks'] . '__' . $insurance_type . '__' . $row['dept_dispensory_id'] . '__' . $row['stock_total_unit'] . '__' . $row['price_markup'] . '__' . $row['hosp_price'] . '__' . $row['buying_cost']; ?>" name="inv_all[]" id="add_m_<?php echo $nn; ?>" />
												</td>
												...
												<td><?php echo $nn . '-' . $row['product_name'];
													...
													<?php
													$setdate = date('Y-m-d');
													if ($setdate == date('Y-m-d', strtotime($row['date_entry']))) { ?><br>
														<small style="color: red;"><?php echo $new_request . $invoiced_qty; ?></small>
													<?php }
													echo $Dispensory;
													?>
												</td>
												...
												<?php if ($_SESSION['edit_price_point_sale'] == 1 and $insurance_type != 'NHIS') { ?>
													<td>
														...
														<input type="number"
															...
															min="1"
															step="0.01"
															value="<?php echo round($row['pay'], 2); ?>" />

													</td>
												<?php } else { ?>
													<input type="hidden" id="edit_price_<?php echo $nn; ?>" name="edit_price_<?php echo $row['sale_sn']; ?>" value="<?php echo number_format($row['pay'], 2); ?>" />
												<?php } ?>
												<td><?php echo number_format($row['pay'], 2); ?></td>
												<td><?php echo number_format($row['claim_amt'], 2); ?></td>
												<td>

													<input type="number" id="qty_<?php echo $nn; ?>" name="qty_<?php echo $row['sale_sn']; ?>" class="form-control"
														onkeyup="UpdateCost()" min="1" value="1" max="<?php if ($qty == 0) {
																											$qty = 1;
																										}
																										echo $qty; ?>" />
												</td>
												...
												<td>
                                                    ...
                                                    &nbsp;|&nbsp; <?php */ ?><a href="index.php?presc&chg=<?php echo $row['sale_sn'] . '/' . $hos_no; ?>&drug_name=<?= $row['product_name']; ?>#target" onclick="return confirm('Are you sure you want to Adapt this drug?')" class="btn btn-warning btn-xs" <?php if ($row['tag'] == '1') { ?>disabled<?php } ?>>Adapt</a>
													<?php } ?>
												</td>
					...
					<?php } else { ?>
						<div><strong style="color:#F00">No Pending Drug(s) for this Patient / <br>You can set Dates & Apply above to see previous data</strong></div>
					<?php } ?>
					...
					$plan = 'plan';
					$setdate = date('Y-m-d');
					$stmt = $db->prepare("SELECT date_entry2,date_entry,prepared_by,no_updates,notes FROM notes WHERE hospital_no = :hos_no AND notes_type = :notes_type AND status = '1' AND date_entry BETWEEN :from_date AND :to_date ORDER BY sn DESC");
					$stmt->bindParam(':hos_no', $hos_no);
					$stmt->bindParam(':notes_type', $plan); // Assuming 'plan' as notes_type value
					$stmt->bindParam(':from_date', $from_date);
					$stmt->bindParam(':to_date', $to_date);
					$stmt->execute();
			...
			<div class="ibox-content">
								...
								<table width="100%" cellpadding="5px;">
									...
									<?php if (isset($_GET['chg'])) { ?>
										<tr>
											<td></td>
											<td>
												<label class="req">Enter New Drug Prescription:</label>
												<input type="text" name="new_precription" class="form-control" required>
											</td>
											<td></td>
											<td></td>
											<td></td>
				...
				if (!isset($_GET['dsp']) and !isset($_GET['hx'])) {
					if (isset($_GET['unpaid'])) {
						$status_filter = " and paystatus=0";
					} elseif (isset($_GET['paid'])) {
						$status_filter = " and paystatus=1";
					} else {
						$status_filter = null;
					}

					$stmt = $db->prepare('SELECT p.serv_group,p.remarks,p.pay,p.claim_amt,p.date_entry,p.transact_date,p.prepared_by,p.pay_mode,p.paystatus,p.qty,p.dsp_by,p.invoice_by,p.invoice_status,p.drug_status,p.item_services,p.sn,p.access,p.cr,p.drug_sn,p.wallet_debt_bill_to_acct,p.dept_dispensory_id,p.dept_id,p.dispensory_status_at_phamcy,p.prescription,s.product_name,s.dosage,s.qty as stock_qty,s.strength FROM patient_ap_services AS p INNER JOIN stock_table AS s ON p.drug_sn = s.sn WHERE hospital_no = :hos_no AND (invoice_status = :invoice_status OR drug_status = :drug_status)' . $status_filter . ' AND serv_group = :serv_group   ' . $patch_Dispens_query . ' ORDER BY invoice_date DESC');
					$stmt->bindParam(':hos_no', $hos_no, PDO::PARAM_STR);
					$stmt->bindValue(':invoice_status', $one, PDO::PARAM_STR);
					$stmt->bindValue(':drug_status', $one, PDO::PARAM_STR);
					...
					if ($stmt->rowCount() > 0) { ?>
						...
						<span style="color:brown; font-size: 14px;"> <b>Filter: Show by </b> </span><a href="index.php?presc&hos_no=<?= $hos_no; ?>&paid#drug-section" class="btn btn-success btn-xs">Paid List</a>
							...
							<form action="index.php?invoice" method="POST">
									...
									<tbody>
										...
										while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
											...
											if ($Dispensory == null) {
												...
												if ($_SESSION['dsp_oncredit'] == 0 && $row['paystatus'] == 0 && $credit_limit > 0) {
												...
												} elseif ($row['claim_amt'] > 0 and $row['paystatus'] == 0) {
													$check = 0;
													$disabled_chk = 'disabled';
										...
										?>
													...
													<tr>
														...
														<td><?php if ($insurance_status == 1 and $row['access'] == 3) {
															...
															<?php
															if ($row['remarks'] == 'pharmacist' and $row['paystatus'] == 0 and $row['drug_status'] == 0) { ?>
																&nbsp;|&nbsp;
															...
															<?php } elseif ($row['paystatus'] == 0 and $row['drug_status'] == 0) { ?>
																...
																<a href="index.php?presc<?php echo '&cl=' . $row['sn'] . '&hos_no=' . $hos_no; ?>" class="btn btn-danger btn-xs">Cancel</a>
															<?php } ?>
								...
								<table width="100%" align="right">
									<tr>
										<td align="right">
											...
											} elseif ($dispense_cr > 0 and $dispense == 0) { ?>
												<button type="button" class="btn btn-danger btn btn-sm" id="dispense_btn"
								...
								<table width="100%">
									<tr>
										<td>
											<div class="form_sep">
												<button class="btn btn-success btn btn-sm" type="submit" name="print_invoice" id="print_invoice"><i class="fa fa-print"></i>&nbsp;Invoiced or Print</button>
						...
						<td>
							<div align="right">
								...
								<?php
								if ($credit_limit > 0 && $show_status == 1) {
									echo '<h3 style="color:red; ">Credit Balance: ' . $credit_limit . '</h3>';
								}
								?>
								...
								<table style="background-color:#CCF;">
									...
									<?php if ($current_balance > 0 or ($claiming > 0)) { ?>
										<tr>
											<td>
												...
												<div align="right" style="font-size:20px; padding-right:10px; padding-bottom:10px;">
													<form action="apply_claim.php?#drug-section" method="POST">
														<?php if ($claiming > 0) { ?>
															...
															$stmt = $db->prepare("SELECT invoice_status FROM patient_ap_services WHERE hospital_no = :hos_no AND invoice_status = '1' AND serv_group = 'Pharmacy' AND date_entry BETWEEN :from_date AND :to_date");
															$stmt->bindParam(':hos_no', $hos_no, PDO::PARAM_STR);
				...
				<?php if (isset($_GET['hx'])) {
					...
					<table class="table table-striped table-bordered table-hover dataTables-example">
						...
						<tbody>
							...
							$stmt = $db->prepare("SELECT item_services,date_entry,remarks,prepared_by,dsp_by,drug_status,claim_amt,pay
			...
			<div class="modal-content">
				...
				<div class="modal-body" id="bio_data_body">
					...
					?>
							...
							<div class="print_presc">
								<table width="100%" height="353" border="0" cellpadding="2" cellspacing="2" id="searchBorder" bgcolor="#FFFFFF">
									<tr>
										<td height="55" colspan="2" width="350">
											<div align="center"><b>
													<h3>DRUG PRESCRIPTION
													</h3>
												</b></div>
									...
									<tr>
										...
										<td width="250" height="30">
											<div style="font-size: 18px;"><strong><?php echo $hosp_no; ?></strong></div>
										</td>
									</tr>
									<tr>
										<td width="100" height="30">
											<div style="font-size: 18px;"><strong>Names</strong>:</div>
										</td>
										<td width="250" height="30">
											<div style="font-size: 18px;"><strong><?php echo $patient_name; ?></strong></div>
										</td>
									</tr>
									<tr>
										<td height="136" colspan="2" width="350">
											<table width="100%" border="0">
												<tr>
													<td height="10%"><b style="font-size: 18px;">#</b></td>
													<td height="45%"><b style="font-size: 18px;">Drug</b></td>
													<td height="45%"><b style="font-size: 18px;">Prescription</b></td>
												</tr>
												...
												?>
													...
													<tr>
														...
														<td>
															<div style="font-size: 18px;"><?php echo $item_services; // ['item_services'];
																							?></div>
														</td>
														<td>
															<div style="font-size: 18px;"><?php echo $remarks; // ['qty'];
																							?></div>
														</td>
													</tr>
												<?php
									...
									<tr>
										...
										<td height="30" width="250">
											<div style="font-size: 18px;"><strong><?php echo date('d-m-Y H:i:s a'); ?></strong></div>
										</td>
									</tr>
									<tr>
										...
										<td height="30" width="170">
											<div style="font-size: 18px;"><strong><?php echo $_SESSION['fullname']; ?></strong></div>
										</td>
									</tr>
								</table>
							</div>
			...
			<div class="modal-content">
				...
				<div class="modal-body" id="med_pop_upbody">
					...
					<div class="sideScrollStyle3">
						...
						$stmt = $db->prepare('SELECT p.serv_group, p.remarks, p.pay, p.claim_amt, p.date_entry, p.prepared_by, p.paystatus, p.qty, p.item_services, p.dsp_by, stock_table.product_name, stock_table.dosage, stock_table.strength, p.sn, p.cr 
						FROM patient_ap_services AS p 
...
<div class="modal inmodal fade" id="in_patient_modal" tabindex="-1" role="dialog" aria-hidden="true" data-keyboard="false" data-backdrop="static">
	<div class="modal-dialog modal-xl">
		<div class="modal-content">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
...
<script>
	...
	function dispense_process(dispense_all_cr, hosp_no) {
		...
		if (dispense_all_cr == 'dispense_all_cr') {
			var rr = confirm("Are you sure you want to Dispense Selected Drug(s) On Credit/Billed to Account?");
		} else {
			rr = true;
		}
