...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $filterSql = " AND DATE(date_entry) BETWEEN :start_date AND :end_date";
...
<body>
    ...
    <?= isset($_GET['hospital_no']) ? '<p style="color:red; ">Set a date range, enter the hospital number, and apply other optional filters.</p>' : ''; ?>
    <?= isset($_GET['hospital_no']) ? 'EMR NUMBER: ' . $_GET['hospital_no'] : ''; ?>
    ...
    <form method="POST" id="filterForm">
        ...
        <label for="hospital_no"><?= isset($_GET['hospital_no']) ? '<p style="color:red; ">Enter Hospital No.</p>' : 'Hospital No:'; ?></label>
        ...
        <?php if (isset($_GET['hospital_no'])) { ?>
            <a href="index.php?presc&hos_no=<?= $_GET['hospital_no']; ?>" style="font-size: 16px;;">Close</a>
        <?php } else { ?>
