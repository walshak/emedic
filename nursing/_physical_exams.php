...
if(isset($_POST['addPhExamination'])) {
    ...
    if(!empty($phe_tbl_column)){
        ...
        $stmt->execute([$app_no, $hospital_no]);
        if($stmt->rowCount() == 0){
            $sql = "INSERT INTO physical_examination (hospital_no, app_no, $phe_tbl_column, doctor_name,date_captured) VALUES (?, ?, ?, ?, now())";
            $stmt = $db->prepare($sql);
            $saved = $stmt->execute([$hospital_no, $app_no, $phe_comment, $_SESSION['fullname']]);
        }else{
   ...
   var phe_app_no =  "<?php echo $appointment_number;?>";
    $(document).on('click', '#____physical_examination_btn____', function() { 
        ...
        $.ajax({
            ...
            method: "POST",
            data: { loadPHEPage: true, hospital_no: "<?php echo $hospital_no;?>", app_no:  "<?php echo $appointment_number;?>"},
