...
class Transplant
{
    ...
    public function save($data)
    {
        ...
        if ($check->rowCount() == 0) {
            ...
            return ['message' => ($save ? 'Request is saved' : 'Request is not saved'),  'save' => $save, "modify" => false, 'error'=> false];
        } else {
    ...
    private function fetch_services($arr = [], $all = false)
    {
        ...
        $sql = "SELECT * FROM $this->service_table WHERE 1 AND category = 'Transplant' AND dept = 9 $where_clause ";
        $stmt = $this->dbCon->prepare($sql);
        $stmt->execute();
        return json_decode(json_encode($all ? $stmt->fetchAll(PDO::FETCH_ASSOC) : $stmt->fetch(PDO::FETCH_ASSOC)));
    }
