...
class DrugStock
{
    ...
    public function distinctDosageForm()
    {
        $sql = "SELECT distinct dosage FROM $this->table WHERE status = 'active'";
    ...
    private function fetch($arr = [], $all = false)
    {
        ...
        $sql = "SELECT * FROM $this->table WHERE status = 'active'   $where_clause ";
        $stmt = $this->dbCon->prepare($sql);
        $stmt->execute();
